      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          listItem.forEach((element) => {
            html += `<option value="${element.MaSize + element.MaDe}">${
              element.TenSize
            } - ${element.TenDe}</option>`;
          });
  ...
  btnThem.addEventListener("click", function (e) {
    ...
    listCTPN.push({
      ...
      TenSize: document
        ...
        .options[
          document.querySelector("#product-item").selectedIndex
        ].text.split(" - ")[0],
      TenDe: document
        .querySelector("#product-item")
        .options[
  ...
  rowdetail.forEach((element, index) => {
    element.addEventListener("click", function () {
      ...
      rowdetail.forEach((element) => {
        ...
        if (
          element.querySelectorAll("td")[0].innerHTML == listCTPN[index].MaSP &&
          element.querySelectorAll("td")[2].innerHTML ==
            listCTPN[index].TenSize &&
          element.querySelectorAll("td")[3].innerHTML == listCTPN[index].TenDe
        ) {
          element.classList.add("black");
        }
      });
      ...
      document.querySelector("#product-name").value =
        listCTPN[index].MaSP + " - " + listCTPN[index].TenSP;
      document.querySelector("#product-name").disabled = true;
      document.querySelector("#product-item").value =
        listCTPN[index].MaSize + listCTPN[index].MaDe;
      document.querySelector("#product-price").value = listCTPN[index].GiaNhap;
      document.querySelector("#product-pricesell").value =
        listCTPN[index].GiaBan;
      document.querySelector("#product-quantity").value =
        listCTPN[index].SoLuong;
    });
  });
}
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.length == 0) document.querySelector("#import-id").value = 1;
        ...
        document.querySelector("#import-id").value =
          parseInt(data[data.length - 1].mapn) + 1;
...
function addeventxoapn() {
  document
    .querySelector(".btn-control-large.item-3")
    .addEventListener("click", function (e) {
      ...
      sum -=
        parseInt(listCTPN[index].GiaNhap) * parseInt(listCTPN[index].SoLuong);
      document.querySelector(".tongtienpn").innerHTML = toVND(sum);
...
function loadcomboboxnhanvien() {
  $.ajax({
    ...
    success: function (data) {
      ...
      data.forEach((element) => {
        html += `<option value="${element.MaNV}">${
          element.Ho + " " + element.Ten
        }</option>`;
      });
